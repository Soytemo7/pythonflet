# pip install flet pygame mutagen
...
class Song:
    ...
    def get_duration(self):
        audio = MP3(os.path.join(canciones_dir, self.filename))
...
async def main(page: ft.Page):
    ...
    print("Ruta carpeta canciones:", canciones_dir)

    # Verifica que exista la carpeta canciones
    if not os.path.exists(canciones_dir):
        page.add(ft.Text("La carpeta 'canciones' no fue encontrada.", color=ft.Colors.RED))
        return

    pygame.mixer.init()

    # Lista de canciones
    playlist = [Song(f) for f in os.listdir(canciones_dir) if f.endswith(".mp3")]

    def load_song():
        pygame.mixer.music.load(os.path.join(canciones_dir,playlist[current_song_index].filename))
    ...
    # Agregar título y listado de canciones
    page.add(columna)    
